	...
	for (auto i = 0; i < R.size() - 2499; i += 2500) {
		...
		for (auto j = 0; j < S.size() ; j++) {
			...
			for (auto k = 0; k < 2500; k+=4) {
				...
				__m128i setR_min_x = _mm_set_epi32(R[i + k].first.first, R[i + k + 1].first.first, R[i + k + 2].first.first, R[i + k + 3].first.first);
				__m128i setR_min_y = _mm_set_epi32(R[i + k].first.second, R[i + k + 1].first.second, R[i + k + 2].first.second, R[i + k + 3].first.second);
				__m128i	setR_max_x = _mm_set_epi32(R[i + k].second.first, R[i + k + 1].second.first, R[i + k + 2].second.first, R[i + k + 3].second.first);
				__m128i	setR_max_y = _mm_set_epi32(R[i + k].second.second, R[i + k + 1].second.second, R[i + k + 2].second.second, R[i + k + 3].second.second);

				//8 Rectangles of set S disintegrated into its smallest elements i.e. coordinate values, which are 32 bit integers. Packed into 4 _m128i datatypes. Each _m128i datatype can store 4 32-bit integers
				...
				for (auto ii = 0; ii < 4; ii++) {
					...
					if (overlap_satisfied_index[ii] == 1) {
						...
						spatial_list.push_back({ R[i + k + ii], S[j] });
						joins_count += 1;
					}
				}
